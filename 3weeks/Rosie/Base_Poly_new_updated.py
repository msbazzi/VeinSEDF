...
class IsoPoleZero(nn.Module):
    ...
    def energy(self, lam: torch.Tensor) -> torch.Tensor:
        ...
        return (self._pz_term(e_th, self._k[0], self._a[0], self._b[0]) +
                ...
                torch.clamp(self.bthz, max=50.0) * torch.clamp(e_th, min=0.0) * torch.clamp(e_z, min=0.0))
...
def main():
    ...
    ap.add_argument("--epochs_warmup", type=int, default=9000)
    ap.add_argument("--joint_ft_epochs", type=int, default=0)

    # LRs
    ap.add_argument("--lr_base",        type=float, default=5e-2)
    ap.add_argument("--lr_resid",       type=float, default=1e-4)
    ap.add_argument("--lr_joint_base",  type=float, default=1e-4)
    ap.add_argument("--joint_ft_lr_base", type=float, default=1e-6)
    ap.add_argument("--joint_ft_lr_resid", type=float, default=1e-6)
    ap.add_argument("--joint_ft_lr", type=float, default=1e-7, help="If >0, use this LR for BOTH base and polynomial during final fine‑tune")
    ...
    ap.add_argument("--recruit_start_init", type=float, default=0.35,
                help="Initial guess or fixed value for the shared recruitment start λ_lb")
    ap.add_argument("--recruit_start_min", type=float, default=0.20,
                help="Lower bound for the learned recruitment start λ_lb")
    ap.add_argument("--recruit_start_max", type=float, default=1.0,
    ...
    ap.add_argument("--wtheta_boost", type=float, default=3.0)
