...
class IsoEq9Quadratic(nn.Module):
    ...
    def energy(self, lam: torch.Tensor) -> torch.Tensor:
        ...
        bthz = torch.clamp(softplus_pos(self.bthz), max=500.0)
...
COMBINED_FIXED_TERMS = [
    ...
    "<E_θ>",                   # iso: 4/12 specimens (Audi, Lexus, Martini, Rosie)
    "<E_z>",                   # iso: 7/12 specimens (most common non-universal)
...
COMBINED_FIXED_TERMS_V2 = [
    "I₁-3", "I₂-3", "<E_θ>", "<E_z>", "<I₂-3>ln(1+<I₂-3>)",
    "<I₄θ-1>",                  # aniso: 2/12 (Martini 8w, Lexus 28w) — linear circ fiber stiffness
    "<I₄z-1>", "<I₄D1-1>", "<I₄D2-1>",
    "(<I₄θ-1>)³",               # aniso: 2/12 (Rosie 3w, Sagittarius 16w; coeff_raw≈39 in Sagittarius)
    "exp(<I₄z-1>)-1",           # aniso: 2/12 (Lucid 3w, Mojito 10w) — early-age stiffening
    "<I₂-3>·<E_z>", "I₈D1D2",
]
...
def small_on_large_ellipticity_loss(model: nn.Module,
                                    lam_all: torch.Tensor,
                                    *,
                                    margin: float = 1e-4,
                                    max_points: int = 32,
                                    branch: str = "theta") -> torch.Tensor:
    """Penalize loss of local ellipticity in the theta-z small-on-large block."""
    if lam_all is None or lam_all.numel() == 0:
        device = next(model.parameters()).device
        return torch.zeros((), dtype=torch.float64, device=device)

    if lam_all.shape[0] > max_points:
        idx = torch.linspace(0, lam_all.shape[0] - 1, max_points, device=lam_all.device).long()
        lam_eval = lam_all.index_select(0, idx)
    else:
        lam_eval = lam_all

    penalties = []
    for lam0 in lam_eval:
        lam = lam0.detach().clone().requires_grad_(True)
        out = model_forward_branch(model, lam.unsqueeze(0), branch=branch, create_graph=True)
        W_val = out[0] if len(out) == 3 else model_forward_branch(
            model, lam.unsqueeze(0), branch=branch, create_graph=True
        )[0]
        Wsum = W_val.sum() if hasattr(W_val, "sum") else W_val
        (dW_dlam,) = torch.autograd.grad(Wsum, lam, create_graph=True)
        ...
        for a in range(2):
            (row,) = torch.autograd.grad(
                dW_dlam[a], lam, create_graph=True, retain_graph=True
            )
            h_rows.append(row[:2])
        H = torch.stack(h_rows, dim=0)

        lam_th, lam_z = lam[0], lam[1]
        dth, dz = dW_dlam[0], dW_dlam[1]
        tth = lam_th * dth
        tz = lam_z * dz

        K_thth = H[0, 0] / lam_th.pow(2) - dth / lam_th.pow(3)
        K_zz = H[1, 1] / lam_z.pow(2) - dz / lam_z.pow(3)
        K_thz = H[0, 1] / (lam_th * lam_z)
        K_zth = H[1, 0] / (lam_z * lam_th)

        C_thth = 2.0 * tth + lam_th.pow(4) * K_thth
        C_zz = 2.0 * tz + lam_z.pow(4) * K_zz
        C_thz = lam_th.pow(2) * lam_z.pow(2) * K_thz
        C_zth = lam_z.pow(2) * lam_th.pow(2) * K_zth
        C_off = 0.5 * (C_thz + C_zth)
        C_block = torch.stack([
            torch.stack([C_thth, C_off]),
            torch.stack([C_off, C_zz]),
        ])

        eigvals = torch.linalg.eigvalsh(C_block)
        penalties.append(torch.relu(margin - eigvals).pow(2).mean())
...
def fit_model(model: nn.Module,
              ...
              *,
              ...
              ellipticity_lambda: float = 0.0,
              ellipticity_margin: float = 1e-4,
              ellipticity_points: int = 32,
              ):
    set_seed(seed)
    ...
    lam_ellip = _lam_all_from_X(Xth, Xz, device=device) if ellipticity_lambda > 0.0 else None
    ...
    for ep in range(1, epochs+1):
        ...
        if ellipticity_lambda > 0.0 and lam_ellip is not None:
            loss = loss + ellipticity_lambda * small_on_large_ellipticity_loss(
                model,
                lam_ellip,
                margin=ellipticity_margin,
                max_points=ellipticity_points,
            )
...
def _fit_variant(variant_key, args, Xth, yth, Xz, yz, wth, wz, in_vivo_lamz, device,
    ...
    if variant_key in list("abcde"):
        ...
        loss, aic, bic, model = fit_model(
            ...
            ellipticity_lambda=args.ellipticity_lambda,
            ellipticity_margin=args.ellipticity_margin,
            ellipticity_points=args.ellipticity_points,
        )
...
def main():
    ...
    ap.add_argument("--epochs_joint",  type=int, default=0,
    ...
    ap.add_argument("--ellipticity_lambda", type=float, default=1e-3,
                help="Penalty weight for negative eigenvalues of the theta-z small-on-large tangent block.")
    ap.add_argument("--ellipticity_margin", type=float, default=1e-4,
                help="Minimum allowed eigenvalue margin, in kPa, for the theta-z tangent block.")
    ap.add_argument("--ellipticity_points", type=int, default=32,
                help="Number of stretch states sampled for the ellipticity penalty each epoch.")
