...
class FiberBase(nn.Module):
    ...
    def __init__(self, nfam=4, denom=4.0, use_dispersion=True, use_recruitment=True, n_quad=14):
        ...
        # recruitment params (shared; set per-family by changing shapes to (nfam,))
        self._sigma_s_raw  = nn.Parameter(torch.full((nfam,), -2.0))  # σs in ~[0.01,0.20]
        self._lambda_lb_raw= nn.Parameter(torch.tensor(-4.0, dtype=torch.float64)) # λlb at in-vivo stretch, mapped to [0.90,1.25]
    ...
    def effective_lambda_lb(self, lam_z: Optional[torch.Tensor] = None, branch: str = "theta") -> torch.Tensor:
        lam_lb_ref = self.lambda_lb()
    ...
    def _recruit_expectation(self, lam_mag: torch.Tensor, lam_z: torch.Tensor, tau_m: torch.Tensor, m: int, branch: str = "theta") -> torch.Tensor:
        ...
        lam_lb_m = self.effective_lambda_lb(lam_z.to(lam_mag), branch=branch).to(lam_mag)
...
class IsoEq9Quadratic(nn.Module):
    ...
    def __init__(self):
        ...
        self.bthz = nn.Parameter(torch.tensor(0.0))
    ...
    def energy(self, lam: torch.Tensor) -> torch.Tensor:
        ...
        bthz = torch.clamp(torch.relu(self.bthz),   max=500.0)
...
class IsoPoleZero(nn.Module):
    def __init__(self):
        ...
        self.bthz = nn.Parameter(torch.tensor(0.0, dtype=torch.float64))
    ...
    def energy(self, lam: torch.Tensor) -> torch.Tensor:
        ...
        return (self._pz_term(e_th, self._k[0], self._a[0], self._b[0]) +
                ...
                torch.clamp(torch.relu(self.bthz), max=5.0) * torch.clamp(e_th, min=0.0) * torch.clamp(e_z, min=0.0))
...
class DecomposedSEDFBase(nn.Module):
    ...
    def energy_parts(self, lam: torch.Tensor, branch: str = "theta"):
        ...
        W_base = W_iso + W_aniso
        return W_base, W_iso, W_aniso
...
class Eq9(FiberBase):
    ...
    def energy_parts(self, lam):
        ...
        bthz = torch.clamp(torch.relu(self.bthz), max=5.0)
...
class SymbolicPreActivationLayer(nn.Module):
    ...
    def forward(self, inv):
        feats = []
        base_keys = ["I₁-3", "I₂-3", "I₄θ-1", "I₄z-1", "I₄D1-1", "I₄D2-1", "E_z", "E_θ"]
...
class SymbolicOperatorLayer(nn.Module):
    ...
    def forward(self, h0_feats):
        ...
        feats = []
        linear_keys = [
            "I₁-3", "I₂-3", "I₄θ-1", "I₄z-1", "I₄D1-1", "I₄D2-1", "E_z", "E_θ",
            "<I₁-3>", "<I₂-3>", "<I₄θ-1>", "<I₄z-1>", "<I₄D1-1>", "<I₄D2-1>", "<E_z>", "<E_θ>",
        ]
        for key in linear_keys:
            feats.append((key, feat_map[key]))

        square_keys = ["<I₁-3>", "<I₂-3>", "<I₄θ-1>", "<I₄z-1>", "<I₄D1-1>", "<I₄D2-1>", "<E_z>", "<E_θ>"]
...
class SymbolicActivationLayer(nn.Module):
    ...
    def __init__(self, exp_keys=None, xlog_keys=None):
        super().__init__()
        self.exp_keys = set(exp_keys or ["E_θ", "E_z", "<E_θ>", "<E_z>"])
        self.xlog_keys = set(xlog_keys or ["<I₁-3>", "<I₂-3>", "<I₄θ-1>", "<I₄z-1>", "<I₄D1-1>", "<I₄D2-1>", "<E_θ>", "<E_z>"])
...
class GatedSEDF(SymbolicCrossMixin, nn.Module):
    ...
    """
    def __init__(self, iso_model="poly", aniso_model="4fiber", use_recruitment=True, use_dispersion=True, n_basis=24,
...
def configure_recruitment_start(fiber_like, start: float, mode: str = "learn"):
    """
    Configure the shared recruitment-start lower bound λ_lb at the in-vivo reference stretch.
    mode='fixed' freezes λ_lb at the requested value.
    mode='learn' initializes λ_lb and lets optimization update it.
    """
    raw = _raw_from_bounded(start, RECRUIT_LB_MIN, RECRUIT_LB_MAX)
    with torch.no_grad():
        fiber_like._lambda_lb_raw.copy_(torch.tensor(raw, dtype=torch.float64, device=fiber_like._lambda_lb_raw.device))
    fiber_like._lambda_lb_raw.requires_grad_(mode == "learn")
    return float(fiber_like.lambda_lb().detach().cpu().item())

def preserve_recruitment_start(fiber_like, mode: str = "learn"):
    """
    Keep the currently loaded recruitment-start value and only update whether it
    should continue learning in later fitting stages.
    """
    fiber_like._lambda_lb_raw.requires_grad_(mode == "learn")
    return float(fiber_like.lambda_lb().detach().cpu().item())
    ...
    if Xth.size:
        ...
        if model is not None:
            ...
            for j,(_,idxs) in enumerate(sorted(groups.items(), key=lambda kv: kv[0])):
                ...
                ax_left.plot(Xth[ii,0], y_pred, color=color, lw=2.5, alpha=1.0, ls=ls,
                             label=(lbl_th if j==0 else None), zorder=2)
                ax_left.scatter(Xth[ii,0], y_pred, color=color, s=28, alpha=0.95,
        ...
        if model is not None:
            ...
            if ids_z is not None:
                ...
                for i,lth in enumerate(lam_theta):
                    key = float(np.round(lth / round_step) * round_step)
                    groups.setdefault(key, []).append(i)
            for j,(_,idxs) in enumerate(sorted(groups.items(), key=lambda kv: kv[0])):
                ...
                ax_right.plot(Xz[ii,1], y_pred, color=color, lw=2.5, alpha=1.0, ls=ls,
                              label=(lbl_z if j==0 else None), zorder=2)
                ax_right.scatter(Xz[ii,1], y_pred, color=color, s=28, alpha=0.95,
                                 edgecolors="white", linewidths=0.4, zorder=3)
...
def plot_results(Xth, yth, Xz, yz, base_model, tweaked_model, age, outdir, round_step=0.01, ids_th=None, ids_z=None):
    ...
    _group_and_plot(axs1[0], axs1[1], Xth, yth, Xz, yz, tweaked_model, "#0066CC", "None", 
    ...
    for ax in axs1:
        ...
        ax.legend(uniq.values(), uniq.keys(), frameon=True, fancybox=True, shadow=True, fontsize=11, loc='best')
...
def main():
    ...
    ap.add_argument("--recruit_start_init", type=float, default=1.35,
    ...
    ap.add_argument("--gate_thr", type=float, default=0.01,
    ...
    # Basis options
    ap.add_argument("--n_basis", type=int, default=24,
