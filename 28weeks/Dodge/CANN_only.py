...
class SymbolicCrossMixin:
    ...
    def init_symbolic_layers(self):
        aniso_module = getattr(self, "aniso", None)
        if aniso_module is None:
            aniso_module = self.base.aniso
        self.invariant_layer = SymbolicInvariantLayer(aniso_module)
...
class PureGatedSEDF(SymbolicCrossMixin, nn.Module):
    """
    Pure symbolic discovery model with no base energy.
    W_total is only the gated symbolic library contribution.
    """
    def __init__(self, aniso_model="4fiber", use_recruitment=True, use_dispersion=True, n_basis=45,
                 init_gate=0.2, gate_temp=1.0, dropout_p: float = 0.0):
        super().__init__()
        self.aniso_model = aniso_model
        self.aniso = build_aniso_module(
            aniso_model,
            use_recruitment=use_recruitment,
            use_dispersion=use_dispersion,
        )
...
def save_equation_to_file(model, base_type, age, out_root, *, weight_thresh: float = 0.01):
    ...
    is_gated_sedf = isinstance(model, (GatedSEDF, PureGatedSEDF))
    is_symbolic_sedf = is_simple_sedf or is_gated_sedf
    
    with open(equation_file, "w") as f:
        title = f"FINAL EQUATION FOR {age} - DISCOVERED MODEL" if isinstance(model, PureGatedSEDF) else f"FINAL EQUATION FOR {age} - {base_type} BASE"
        f.write(title + "\n")
        f.write("="*60 + "\n\n")
        
        if is_composite or is_symbolic_sedf:
            # Decomposed model: fit isotropic and anisotropic parameters directly
            base = model.base if hasattr(model, 'base') else None
            if base is not None:
                f.write("Base Decomposition:\n")
                f.write(f"  isotropic model   = {base.iso_model}\n")
                f.write(f"  anisotropic model = {base.aniso_model}\n\n")
            else:
                f.write("Pure Discovery Model:\n")
                f.write("  base energy       = none\n")
                f.write(f"  directional model = {model.aniso_model} symbolic invariants\n\n")
        ...
        if not (is_composite or is_symbolic_sedf):
            use_pg = getattr(model, "use_poly_gate", True)
            if use_pg:
                w_low  = model.poly_head_low.weight.detach().cpu().numpy().flatten()
                w_high = model.poly_head_high.weight.detach().cpu().numpy().flatten()
        ...
        if is_composite or is_symbolic_sedf:
            base = model.base if hasattr(model, 'base') else None
            if base is None:
                f.write(f"W_total = W_discovered\n\n")
            else:
                f.write(f"W_total = W_base + W_cross\n\n")
            
            f.write(f"where:\n")
            if base is None:
                f.write(f"  W_discovered = Σ((g_i·w_i) · ψ_i)\n")
                f.write(f"  ψ_i are generated from strain and {model.aniso_model} directional invariants\n")
            else:
...
def plot_results(Xth, yth, Xz, yz, base_model, tweaked_model, age, outdir, round_step=0.01, ids_th=None, ids_z=None):
    ...
    _group_and_plot(axs1[0], axs1[1], Xth, yth, Xz, yz, tweaked_model, "#0066CC", "--",
                    r"$\Psi_{discovered}$", r"$\Psi_{discovered}$", round_step, device, ids_th=ids_th, ids_z=ids_z)
    fig1.suptitle(f"Age {age}: Experimental vs Discovered Model", fontsize=18, weight="bold")
...
def create_equation_comparison_plots(
    ...
    *,
    ...
    if model is not None:
        models_to_compare.append((model, "Discovered Model", "red"))
    ...
    fig.suptitle(f'Equation Comparison: {age}', fontsize=16, fontweight='bold')
...
def plot_weight_significance(ax, model):
    ...
    if is_composite or is_simple_sedf or is_gated_sedf:
...
def main():
    ...
    # Epochs
    ap.add_argument("--epochs_base",   type=int, default=40000)
    ap.add_argument("--epochs_warmup", type=int, default=4000)
    ap.add_argument("--joint_ft_epochs", type=int, default=0)

    # LRs
    ap.add_argument("--lr_base",        type=float, default=5e-2)
    ap.add_argument("--lr_resid",       type=float, default=5e-2)
    ap.add_argument("--lr_joint_base",  type=float, default=5e-2)
    ap.add_argument("--joint_ft_lr_base", type=float, default=1e-3)
    ap.add_argument("--joint_ft_lr_resid", type=float, default=1e-3)
    ap.add_argument("--joint_ft_lr", type=float, default=5e-2, help="If >0, use this LR for BOTH base and polynomial during final fine‑tune")
    ...
    ap.add_argument("--stage2_term_dropout", type=float, default=0.05,
    ...
    ap.add_argument("--wtheta_boost", type=float, default=3.5)
    ...
    ap.add_argument("--max_iso_terms", type=int, default=4,
                help="Maximum number of symbolic isotropic correction terms to retain")
    ap.add_argument("--max_aniso_terms", type=int, default=4,
                help="Maximum number of symbolic anisotropic correction terms to retain")
    ap.add_argument("--max_cross_terms", type=int, default=5,
    ...
    for age in ages:
        ...
        print(f"→ Building pure discovery model ({args.aniso_model} directional invariants, no base energy)")
        gated = PureGatedSEDF(
        ...
        if hasattr(gated.aniso, "dist_type"):
            gated.aniso.dist_type = args.dist
        if hasattr(gated.aniso, "_recruit_ref_stretch"):
            ref_lamz = configure_recruitment_reference_stretch(gated.aniso, in_vivo_lamz)
            print(f"→ Recruitment reference stretch set to λz,ref={ref_lamz:.3f}")
        if hasattr(gated.aniso, "_lambda_lb_raw"):
            recruit_start = configure_recruitment_start(
                gated.aniso, args.recruit_start_init, args.recruit_start_mode
            )
            print(f"→ Recruitment start λ_lb ({args.recruit_start_mode}) initialized to {recruit_start:.3f}")
        if hasattr(gated.aniso, "_beta_a_raw"):
            with torch.no_grad():
                init = {0:(2.5,3.0), 1:(2.5,3.2), 2:(2.5,4.0), 3:(2.6,2.0)}
                n_fam = len(gated.aniso._beta_a_raw)
                for fam, (a,b) in init.items():
                    if fam < n_fam:
                        gated.aniso._beta_a_raw[fam] = _raw_from_alpha(a)
                        gated.aniso._beta_b_raw[fam] = _raw_from_alpha(b)

        print("→ Gated discovery (learn symbolic energy directly)")
        lam_all = _lam_all_from_X(Xth, Xz, device=device)
        with torch.no_grad():
        ...
        if args.joint_ft_epochs > 0:
            print("→ Fine-tuning discovered symbolic model")
        ...
        metric_rows = [
            compute_fit_metrics(
                final_model, Xth, yth, Xz, yz,
                label="Discovered Model", loss=final_loss, aic=final_aic, bic=final_bic
            ),
        ]
        ...
        plot_results(Xth, yth, Xz, yz, None, final_model, age, args.out_root, round_step=args.round_step, ids_th=ids_th, ids_z=ids_z)
        for model, tag in [(final_model, "DISCOVERED")]:
