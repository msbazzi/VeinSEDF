...
COMBINED_FIXED_TERMS = [
    "I₂-3",           # iso: 3+/12 specimens (Mojito, Pablo, Dodge)
    "I₁-3",           # iso: 3+/12 specimens (Mojito, Pablo, Dodge)
   # "<E_θ>",           # iso: 3+/12 specimens (Martini, Audi, Lexus)
   # "<E_z>",           # iso: 6/12 specimens (most common non-universal)
    "<I₂-3>ln(1+<I₂-3>)",  # iso/cross correction: nonlinear I2 contribution
   # "<I₄z-1>",         # aniso: 5/12 specimens
   # "<I₄D1-1>",        # aniso: 4/12 specimens
   # "<I₄D2-1>",        # aniso: 4/12 specimens
   # "<I₂-3>·<E_z>",    # cross: 3/12 specimens (Mojito, Sagittarius, Mercedes)
...
class CombinedEquationSEDF(SymbolicCrossMixin, nn.Module):
    ...
    Cross: fixed 9 symbolic terms consistently selected across specimens
             iso   : I2-3, <E_theta>, <E_z>, <I2-3>ln(1+<I2-3>)
             aniso : <I4z-1>, <I4D1-1>, <I4D2-1>
             cross : <I2-3>·<E_z>, I8D1D2

    Equivalent to SimpleSEDF with pre-determined active_idx inferred from COMBINED_FIXED_TERMS.
    ...
    def __init__(self, n_basis: int = 45,
        ...
        active_idx = [lib_names.index(t) for t in COMBINED_FIXED_TERMS if t in lib_names]
        if not active_idx:
            raise RuntimeError("None of COMBINED_FIXED_TERMS found in symbolic library.")
        self.tweak = TweakLinear(n_basis, active_idx)
...
def _fit_variant(variant_key, args, Xth, yth, Xz, yz, wth, wz, in_vivo_lamz, device,
    ...
    if variant_key in list("abcde"):
        ...
        # Stage 2: freeze base, learn fixed 8-term cross correction ---
...
def run_comparison(args, age, Xth, yth, Xz, yz, wth, wz, ids_th, ids_z,
    ...
    for vkey in ("f", "g"):
        if vkey in variant_models:
            _save_combined_equation(variant_models[vkey][0], vkey, age, args.out_root)
...
def main():
    ...
    ap.add_argument("--joint_ft_epochs", type=int, default=3000)
    ...
    ap.add_argument(
        "--variant",
        choices=list("abcdefg") + ["comparison", "discovery"],
        default="comparison",
        help=(
            "discovery (default): run the full gated-symbolic pipeline. "
            "comparison: fit all 7 variants (a-g) and save a unified table. "
    ...
    for age in ages:
        ...
        # --- Default: existing gated-symbolic discovery pipeline (--variant discovery) ---
