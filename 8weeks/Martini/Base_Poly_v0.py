...
class IsoEq9Quadratic(nn.Module):
    ...
    def energy(self, lam: torch.Tensor) -> torch.Tensor:
        ...
        E_z  = 0.5 * (lam[...,1]**2 - 1.0)
        bth  = torch.clamp(softplus_pos(self._bth), max=1000.0)
        bz   = torch.clamp(softplus_pos(self._bz),  max=1000.0)
...
def main():
    ...
    ap.add_argument("--recruit_start_init", type=float, default=1.15,
                help="Initial guess or fixed value for the shared recruitment start λ_lb")
    ap.add_argument("--recruit_start_min", type=float, default=0.40,
                help="Lower bound for the learned recruitment start λ_lb")
    ap.add_argument("--recruit_start_max", type=float, default=1.35,
                help="Upper bound for the learned recruitment start λ_lb")
    ap.add_argument("--recruit_end_min", type=float, default=0.90,
                help="Lower bound for the recruitment upper stretch λ_ub")
    ap.add_argument("--recruit_end_max", type=float, default=1.50,
    ...
    ap.add_argument("--gate_thr", type=float, default=0.00001,
