...
class FiberBase(nn.Module):
    ...
    def fiber_sum(self, C: torch.Tensor, branch: str = "theta") -> torch.Tensor:
        ...
        for m, n in enumerate(dirs):
            ...
            k2  = torch.clamp(softplus_pos(self._k2[m]), max=100.0)
            I4m = I4(C, n)
            ...
            arg = torch.clamp(xi**2, 0.0, 500.0)
...
class IsoEq9Quadratic(nn.Module):
    ...
    def energy(self, lam: torch.Tensor) -> torch.Tensor:
        ...
        E_z  = 0.5 * (lam[...,1]**2 - 1.0)
        bth  = torch.clamp(softplus_pos(self._bth), max=5000.0)
        bz   = torch.clamp(softplus_pos(self._bz),  max=5000.0)
        bthz = torch.clamp(self.bthz, max=5000.0)
...
class IsoPoleZero(nn.Module):
    ...
    def energy(self, lam: torch.Tensor) -> torch.Tensor:
        ...
        return (self._pz_term(e_th, self._k[0], self._a[0], self._b[0]) +
                ...
                torch.clamp(self.bthz, max=5000.0) * torch.clamp(e_th, min=0.0) * torch.clamp(e_z, min=0.0))
...
def main():
    ...
    # Epochs
    ap.add_argument("--epochs_base",   type=int, default=10000)
    ap.add_argument("--epochs_warmup", type=int, default=10000)
    ap.add_argument("--joint_ft_epochs", type=int, default=2000)
    ...
    ap.add_argument("--recruit_start_init", type=float, default=1.55,
    ...
    ap.add_argument("--recruit_start_max", type=float, default=1.95,
                help="Upper bound for the learned recruitment start λ_lb")
    ap.add_argument("--recruit_end_min", type=float, default=1.35,
                help="Lower bound for the recruitment upper stretch λ_ub")
    ap.add_argument("--recruit_end_max", type=float, default=2.2,
    ...
    ap.add_argument("--gate_thr", type=float, default=0.0001,
    ...
    ap.add_argument("--stage2_base_lr", type=float, default=5e-3,
    ...
    ap.add_argument("--stage2_term_dropout", type=float, default=0.001,
