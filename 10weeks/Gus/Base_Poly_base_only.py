...
class GatedTweakLinear(nn.Module):
    ...
    def __init__(self, n_basis: int, init_gate: float = 0.2, temp: float = 1.0, dropout_p: float = 0.0):
        ...
        self.register_buffer("_temp", torch.tensor(float(temp), dtype=torch.float64))
        self.dropout_p = float(np.clip(dropout_p, 0.0, 0.95))
        self.register_buffer("_active_mask", torch.ones(n_basis, dtype=torch.float64))

    @property
    def temp(self):
        return float(self._temp.detach().cpu().item())

    @temp.setter
    def temp(self, value):
        with torch.no_grad():
            self._temp.fill_(float(value))
    
    def gates(self):
        return torch.sigmoid(self.logits / self._temp.clamp_min(1e-8))  # (n_basis,)
    ...
    def set_active_idx(self, active_idx):
        ...
        mask = torch.zeros(self.n_basis, dtype=self._active_mask.dtype, device=self._active_mask.device)
        if len(active_idx):
            idx = torch.as_tensor(active_idx, dtype=torch.long, device=self._active_mask.device)
            mask[idx] = 1.0
        self._active_mask.copy_(mask)
    
    @torch.no_grad()
    def clear_active_mask(self):
        self._active_mask.fill_(1.0)
    
    def forward(self, basis_feats):  # (..., n_basis)
        g = self.gates()
        eff_w = g * self.w * self._active_mask
...
class SymbolicCrossMixin:
    ...
    def symbolic_library_raw(self, lam):
        ...
        cross_terms = [
            ...
            ("<I₄θ-1>·<I₄D1-1>", "cross"),      # circumferential × first helical
...
def plot_weight_significance(ax, model):
    ...
    if is_composite:
        ...
        if is_gated_sedf:
            ...
            w_vals = model.tweak.w.detach().cpu().numpy()
            active_mask = getattr(model.tweak, "_active_mask", None)
            if active_mask is not None:
                active_mask = active_mask.detach().cpu().numpy()
            else:
                active_mask = np.ones_like(w_vals)
            gated_weights = gates * np.abs(w_vals) * active_mask
            basis_names_full = get_basis_names(model, fallback_n=len(gated_weights))
            active_idx = list(getattr(model, "selected_active_idx", []))
            if active_idx:
                weights = np.asarray([gated_weights[i] for i in active_idx], dtype=float)
...
def main():
    ...
    ap.add_argument("--joint_ft_epochs", type=int, default=1000)

    # LRs
    ap.add_argument("--lr_base",        type=float, default=5e-2)
    ap.add_argument("--lr_resid",       type=float, default=5e-2)
    ap.add_argument("--lr_joint_base",  type=float, default=1e-2)
    ap.add_argument("--joint_ft_lr_base", type=float, default=1e-3)
    ap.add_argument("--joint_ft_lr_resid", type=float, default=1e-3)
    ap.add_argument("--joint_ft_lr", type=float, default=1e-2, help="If >0, use this LR for BOTH base and polynomial during final fine‑tune")
    ...
    ap.add_argument("--recruit_start_min", type=float, default=0.50,
                help="Lower bound for the learned recruitment start λ_lb")
    ap.add_argument("--recruit_start_max", type=float, default=1.65,
                help="Upper bound for the learned recruitment start λ_lb")
    ap.add_argument("--recruit_end_min", type=float, default=1.0,
                help="Lower bound for the recruitment upper stretch λ_ub")
    ap.add_argument("--recruit_end_max", type=float, default=1.90,
    ...
    ap.add_argument("--gate_thr", type=float, default=1e-5,
    ...
    ap.add_argument("--wtheta_boost", type=float, default=2.5)
    ...
    ap.add_argument("--base_only", action="store_true",
                help="Fit only the stage-1 base model and skip gated/poly discovery")
    ...
    for age in ages:
        ...
        base_loss, base_aic, base_bic, base_model = fit_model(
            ...
            name=f"{args.iso.upper()}_{args.aniso_model.upper()}-base",
            lr=args.lr_base,
            wtheta_boost=args.wtheta_boost,
            jitter=0.0,
            jitter_warm_epochs=0,
            seed=args.seed,
            weights_th=wth,
            weights_z=wz,
        ...
        if args.base_only:
            ...
            path = os.path.join(args.out_root, f"{age}_{args.iso}_{args.aniso_model}_BASE_metrics.txt")
            with open(path, "w") as f:
                f.write(f"age={age}\n")
                f.write(f"iso={args.iso}\n")
                f.write(f"aniso_model={args.aniso_model}\n")
                f.write(f"recruit={args.recruit}\n")
                f.write(f"dispersion={args.dispersion}\n")
                f.write(f"dist={args.dist}\n")
                f.write(f"seed={args.seed}\n")
                f.write(f"lr_base={args.lr_base}\n")
                f.write(f"epochs_base={args.epochs_base}\n")
                f.write(f"recruit_start_mode={args.recruit_start_mode}\n")
                f.write(f"recruit_start_init={args.recruit_start_init}\n")
                f.write(f"loss={base_loss}\n")
                f.write(f"aic={base_aic}\n")
                f.write(f"bic={base_bic}\n")

            torch.save(base_model.state_dict(),
                    os.path.join(args.out_root, f"{age}_{args.iso}_{args.aniso_model}_BASE_state.pt"))

            print(f"→ Base-only run complete. Metrics saved to {path}")
            continue
        ...
        gated.tweak.set_active_idx(active)
        final_model = gated
        print(f"→ Pruned final symbolic model to {len(active)} active terms")

        # Optional extra joint fine-tune of the stage-2 model only
        if args.joint_ft_epochs > 0:
            print("→ Fine-tuning pruned stage-2 model (base + selected tweak terms together)")
