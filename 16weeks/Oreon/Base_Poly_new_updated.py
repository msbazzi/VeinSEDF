...
class FiberBase(nn.Module):
    ...
    def logn_sig(self):      # σ ∈ [0.05, 0.60]
        s = torch.sigmoid(self._logn_sig_raw); return 0.07 + 0.48*s         # (nfam,)
...
class IsoEq9Quadratic(nn.Module):
    ...
    def energy(self, lam: torch.Tensor) -> torch.Tensor:
        ...
        E_th_pos = smooth_pos(E_th,  0.07)
        E_z_pos  = smooth_pos(E_z,  0.07)
...
def main():
    ...
    ap.add_argument("--wtheta_boost", type=float, default=4.0)
